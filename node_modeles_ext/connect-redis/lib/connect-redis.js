...
module.exports = function (session) {
  ...
  RedisStore.prototype.get = function (sid, fn) {
    ...
    store.client.get(psid, function (er, data) {
      ...
      try {
        //encrypted_session_data|s:754:
          data = data.replace(/encrypted_session_data\|s:\d+:/, '');  //ec_m_add:160619
          data = data.substr(1,data.length-3);//ec_m_add:160619
  ...
  RedisStore.prototype.set = function (sid, sess, fn) {
    ...
    try {
      var jsess = store.serializer.stringify(sess);
        //add encrypted_session_data|s:754:
        jsess = 'encrypted_session_data|s:'+jsess.length+':"' + jsess + '";'; //ec_m_add:160619
    }
    catch (er) {
      return fn(er);
    }

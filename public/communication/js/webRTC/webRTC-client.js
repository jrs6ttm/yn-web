...
var WebRTC = function(options) {
    ...
    /*************************服务器连接部分***************************/
        ...
        //本地连接信道，信道为websocket
    webrtc.prototype.connect = function(server, room, userId, userName) {
        var socket,
            that = this;
        room = room || "";
        socket = this.socket = io.connect(socketPort);
        ...
        socket.on('repeat_enter', function() {
            that.emit('repeat_enter');
        });

        socket.on('overcrowding', function() {
            that.emit('overcrowding');
        });
    ...
    webrtc.prototype.sendOffers = function() {
        ...
        //for (id in that.connections) {
        //    if (id = that.socket.id) {
        //        continue;
        //    }
        //    pc = that.peerConnections[that.connections[id]];
        //    console.log(that.connections[id]);
        //    console.log(pc);
        //    pc.createOffer(pcCreateOfferCbGen(pc, that.connections[id]), pcCreateOfferErrorCb);
        //}
    };
    ...
    webrtc.prototype.createPeerConnections = function() {
        ...
        //        continue;
        //    }
        //    this.createPeerConnection(this.connections[id]);
        //}
    };
    ...
    webrtc.prototype.createPeerConnection = function(socketId) {
        ...
        pc.onicecandidate = function(evt) {
            ...
            //    }
            //}));
                that.socket.emit('ice_candidate', {
                    label :  evt.candidate.sdpMLineIndex,
                    candidate : evt.candidate.candidate,
                    socketId : socketId
                });
        ...
        pc.onaddstream = function(evt) {
            that.emit('pc_add_stream', evt.stream, socketId, users[mapTable[socketId]]);
        };
    ...
    webrtc.prototype.addDataChannel = function(socketId, channel) {
        ...
        channel.onmessage = function(message) {
            ...
            if (json.type === '__file') {
                ...
                that.emit('data_channel_message', channel, socketId, json.data, users[mapTable[socketId]]);
            }
        };
    ...
    /***********************接收者部分***********************/
        ...
        //接收到文件碎片
    webrtc.prototype.receiveFileChunk = function(data, sendId, socketId, last, percent) {
        var that = this,
    ...
    //发送同意接收文件信令
    webrtc.prototype.sendFileAccept = function(sendId) {
        var that = this,
            fileInfo = that.receiveFiles[sendId],

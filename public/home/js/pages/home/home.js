            ...
            if(val === 'prv'){
                ...
                $(me.coursesData[val]).each(function(i,course){
                    ...
                    if(course.fileIcon && course.fileIcon != 'undefined'&& course.fileIcon != 'null'){
                        courseImg    = createEle('img');courseImg.title = courseImg.alt = course.courseName;courseImg.src = filePort + '/ec_engine/fileManager/fileRead?userId=' +JSON.parse(course.fileIcon).ownerId +'&filePath='+JSON.parse(course.fileIcon).filePath;courseImg.className = '_home_courseImg';courseImg_a.appendChild(courseImg);
                    }else{

/**
 * 课程开始逻辑变更为：
 * 1，只有按组织学习，授权学习课程和组织课程，和付费学习三种途径。
 * 2，老师无权随意组织课程和学习课程，要课程开发者授权才行。
 * @param courseId
 * @param view
 * @constructor
 */
...
Database.prototype = {
    ...
    getAuthCourseData : function(next){
        var me = this;
        me.orgData = '';
        if(!userData || !userData.id){
            next();
...
View.prototype = {
  ...
  /*  addOrgCourseBtn : function(){
    ...
    createStartCourse : function(manageBtn){
        ...
        var authInfo= me.database.authInfo;
        ...
        if(!instanceDatas.length){
            ...
            //如果进了课程的组织里，则开放"组织学习"按钮，可以以组织的形式学习课程
            if(orgData){
                line++;
                selectCon = createEle('div');selectCon.className = '_course_selectCon';startLeft.appendChild(selectCon);
                selectMain = createEle('h1');selectCon.appendChild(selectMain);
        ...
        //如果有学完的课程，则开放"查看课程"按钮，可以查看学过的课程内容
        if(lastCourseArr.length){
            line++;
            selectCon = createEle('div');selectCon.className = '_course_selectCon';startLeft.appendChild(selectCon);
            selectMain = createEle('h1');selectCon.appendChild(selectMain);
    ...
    initManageBtn : function(){
        ...
        if(userData && userData.id){
            ...
            if(!me.database.orgData && !me.database.authInfo){//没有此课程的组织学习权限，也没有被授权学习或组织该课程
                ...
                manageBtn.className = '_canNot _course_manageBtn';
                $(manageBtn).click(function(){
                    $.MsgBox.Alert('禁止操作','您尚未获得本课程的授权，无法开始课程！');
                });
            }else{
                manageBtn.innerHTML = "开始课程";
    ...
    createManageEle : function(){
        ...
        me.database.getOrgData(function(){//获取我在此课程的组织信息
            me.database.getAuthCourseData(function(){//获取我在此课程的授权信息
